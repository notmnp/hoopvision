...
// NBA numeric team ids keyed by the CURRENT franchise's Basketball-Reference
// abbreviation (BR spells a few differently: BRK, CHO, PHO). Relocated/historical
// identities (SEA, WSB, NJN, CHH, …) intentionally have no entry — the CDN only
// serves current marks, so those fall back to a muted team-color monogram.
const NBA_TEAM_ID_BY_ABBR: Record<string, number> = {
  ATL: 1610612737,
  BOS: 1610612738,
  BRK: 1610612751,
  CHO: 1610612766,
...
/**
 * Logo URL for a Basketball-Reference team abbreviation, or null when the abbr
 * is a historical/relocated identity with no current CDN mark (callers fall back
 * to a muted-color abbreviation monogram).
 */
export function getTeamLogoUrlByAbbr(
  abbreviation: string | null | undefined
): string | null {
  if (!abbreviation) return null
  return getTeamLogoUrl(NBA_TEAM_ID_BY_ABBR[abbreviation.toUpperCase()])
}
